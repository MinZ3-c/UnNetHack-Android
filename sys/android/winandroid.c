...
extern char **android_saved_games(void);
static boolean android_automenu = TRUE;
static int nhcolor_to_RGB(int);
...
#endif /* MENU_COLOR */
void and_add_menu(winid wid, int glyph, int cnt, const ANY_P *ident, char accelerator, char groupacc, int attr, const char *str, unsigned int preselected)
{
	int tile, color;
	if(glyph == NO_GLYPH)
		tile = -1;
	...
	jbyteArray jstr = create_bytearray(str);
	JNICallV(jAddMenu, wid, tile, (jlong)(intptr_t)ident->a_void, (int)accelerator, (int)groupacc, 1<<attr, jstr, (int)preselected, color);
	destroy_jobject(jstr);
}
	...
	*selected = 0;
	...
	if(n > 1) // n should always be 2k (id, count) pairs
	{
		...
		*selected = (MENU_ITEM_P*)malloc(sizeof(MENU_ITEM_P) * n);
		for(i = 0; i < n; i++)
		{
			(*selected)[i].item.a_void = (void *)(intptr_t)*p++;
			(*selected)[i].count = *p++;
		}
		(*jEnv)->ReleaseLongArrayElements(jEnv, a, q, JNI_ABORT);
	}
	else if(n == 1)
	{
		// special case: ABORT
...
void and_print_glyph(winid wid, coordxy x, coordxy y, int glyph, int bkglyph)
{
//	debuglog("and_print_glyph wid=%d %dx%d", wid, x, y);
	...
	/* ForkFront Tileset.OVERLAY_* uses NetHack's newer bit layout.
	 * Translate supported flags instead of passing UnNetHack's MG_* bits:
	 * MG_PET would otherwise mean DETECT, and MG_RIDDEN would mean PET. */
	unsigned int overlay = 0;
	if ((special & MG_PET) && iflags.wc_hilite_pet)
		overlay |= 0x00010; /* OVERLAY_PET */
	if (special & MG_DETECT)
		overlay |= 0x00008; /* OVERLAY_DETECT */
	if (special & MG_ENGRAVING)
		overlay |= 0x00200; /* OVERLAY_BW_ENGR */
	int uni = get_unicode_codepoint(ch);
	JNICallV(jPrintTile, wid, x, y, tile, uni, nhcolor_to_RGB(col), overlay);
}
...
char and_yn_function(const char *question, const char *choices, char def)
{
	...
	if(choices)
	{
		...
		const char *escape = strchr(choices, '\033');
		esc = escape ? (int)(escape - choices) : -1;
	}
	else
	{
		esc = -1;
	...
	if(android_automenu && choices && nChoices <= 4 && esc < 0 && !allow_num)
	{
		// pop up dialog
...
void and_askname()
{
	...
	char** saves = android_saved_games();
