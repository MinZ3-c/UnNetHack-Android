...
void and_display_nhwindow(winid, boolean);
...
void and_display_file(const char *, boolean);
void and_start_menu(winid);
void and_add_menu(winid,int,int,const ANY_P *, char,char,int,const char *, unsigned int);
...
#endif
void and_print_glyph(winid,coordxy,coordxy,int,int);
void and_raw_print(const char *);
void and_raw_print_bold(const char *);
int and_nhgetch(void);
int and_nh_poskey(coordxy *, coordxy *, int *);
void and_nhbell(void);
int and_doprev_message(void);
char and_yn_function(const char *, const char *, char);
