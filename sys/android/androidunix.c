...
#include <errno.h>
#include <sys/stat.h>
extern boolean android_recover_checkpoint(void);
...
getlock()
{
	...
	snprintf(lockpath, sizeof lockpath, "%s", fqname(lock, LEVELPREFIX, 0));
	...
	if (android_recover_checkpoint()) goto gotlock;
	/* Failed recovery must not erase the existing checkpoint. */
	unlock_file(HLOCK);
	raw_print("An interrupted game exists. Its files have been preserved.");
	nh_terminate(EXIT_FAILURE);
	return;
